...
% wd_coefficient: regularzation parameters
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  d_hid_to_class = (class_input - data.targets) * hid_output';
  d_input_to_hid = model.hid_to_class' * (class_input - data.targets) .* hid_output .* (1 - hid_output) * data.inputs';
  ret.hid_to_class = 1 / N * d_hid_to_class + wd_coefficient * model.hid_to_class;
  ret.input_to_hid = 1 / N * d_input_to_hid + wd_coefficient * model.input_to_hid;
end
...
function ret = initial_model(n_hid)
  n_params = (256+10) * n_hid; % 256 input units, 10 output units
