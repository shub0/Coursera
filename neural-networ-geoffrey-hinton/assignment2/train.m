...
num_embedding = 50;  % Dimensionality of embedding space; default = 50.
num_hid = 200;  % Number of units in hidden layer; default = 200.
...
word_embedding_weights = init_wt * randn(vocab_size, num_embedding);
embed_to_hid_weights = init_wt * randn(numwords * num_embedding, num_hid);
hid_to_output_weights = init_wt * randn(num_hid, vocab_size);
hid_bias = zeros(num_hid, 1);
output_bias = zeros(vocab_size, 1);

word_embedding_weights_delta = zeros(vocab_size, num_embedding);
word_embedding_weights_gradient = zeros(vocab_size, num_embedding);
embed_to_hid_weights_delta = zeros(numwords * num_embedding, num_hid);
hid_to_output_weights_delta = zeros(num_hid, vocab_size);
hid_bias_delta = zeros(num_hid, 1);
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    % embed_to_hid_weights_gradient = zeros(num_embedding * numwords, num_hid);
    ...
    % FILL IN CODE. Replace the line below by one of the options.
    % hid_bias_gradient = zeros(num_hid, 1);
    ...
    % FILL IN CODE. Replace the line below by one of the options.
    % back_propagated_deriv_2 = zeros(num_hid, batchsize);
    ...
    for w = 1:numwords
       word_embedding_weights_gradient = word_embedding_weights_gradient + ...
         ...
         (back_propagated_deriv_2(1 + (w - 1) * num_embedding : w * num_embedding, :)');
    end
